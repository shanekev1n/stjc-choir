...
function switchTab(tab) {
  ...
  if (tab === 'masses') {
    ...
    showScreen('screenAdmin', 'Admin Panel', false);
    renderAdminPanel();
  }
}
...
let selectedPart  = '';   // '' = all parts

function renderSearchPartChips() {
  const el = document.getElementById('searchPartChips');
  if (!el) return;
  const parts = ['', ...MASS_PARTS];
  el.innerHTML = parts.map(p => `
    <div class="search-chip ${p === selectedPart ? 'search-chip-active' : ''}"
      onclick="selectSearchPart('${p}')">
      ${p === '' ? 'All Parts' : p}
    </div>`).join('');
}

function selectSearchPart(part) {
  selectedPart = part;
  renderSearchPartChips();
  ...
  if (q.length >= 2) doSearch(q);
    ...
    // Show all songs for this part even without a query
    doSearch('');
  } else {
    document.getElementById('searchResults').innerHTML = `
      <div class="search-empty">
...
async function doSearch(q) {
  try {
    ...
    if (q.length >= 2) query += `&song=ilike.*${encodeURIComponent(q)}*`;
    if (selectedPart)  query += `&part=eq.${encodeURIComponent(selectedPart)}`;
    // Must have at least one filter
    if (q.length < 2 && !selectedPart) {
      document.getElementById('searchResults').innerHTML = `<div class="search-empty"><span class="search-empty-icon">🎵</span>Type a song name or select a part to search</div>`;
      return;
    }
    const results = await sb('mass_songs', 'GET', null, query);

    const container = document.getElementById('searchResults');
    if (!results || results.length === 0) {
      const emptyMsg = selectedPart && !q
        ? `No songs found for part "<strong>${esc(selectedPart)}</strong>"`
        : `No songs found for "<strong>${esc(q)}</strong>"${selectedPart ? ` in ${esc(selectedPart)}` : ''}`;
      container.innerHTML = `<div class="search-empty"><span class="search-empty-icon">🔍</span>${emptyMsg}</div>`;
      return;
    }
