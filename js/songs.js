...
async function openSongEdit(songId) {
  ...
  // A part is "default" only if it's in MASS_PARTS AND has no custom sort_order
  // If it was deleted and re-added as custom, sort_order will be set → treat as custom
  const removeWrap = document.getElementById('removePartWrap');
  if (removeWrap && canEdit()) {
    const isDefault = MASS_PARTS.includes(song.part) && song.sort_order == null;
  ...
  massIsDirty = true; // user opened a song to edit
  setTimeout(() => {
    document.querySelector('#songModal .modal-sheet').scrollTop = 0;
  }, 10);
}


async function removeCurrentPart() {
  if (!editingSongId) return;
  const songIdToDelete = editingSongId; // capture before closeModal clears it
  const songs = await sb('mass_songs', 'GET', null, `?id=eq.${songIdToDelete}&select=part`);
  const partName = songs[0]?.part || 'this part';

  // Use the app's own confirm dialog instead of browser confirm()
  closeModal();
  showDeleteConfirm(
    null, // no mass delete
    partName,
    async () => {
      await sb(`mass_songs?id=eq.${songIdToDelete}`, 'DELETE');
...
async function searchPreviousSong(q) {
  try {
    ...
    document.getElementById('autofillSongName').textContent = found.song.trim();
...
function applyAutofill() {
  ...
  // Paste song name too
  document.getElementById('mSong').value = autofillData.song || '';
...
function closeModal() {
  ...
  editingSongId = null;
  autofillData  = null;
  massIsDirty   = false; // cancelled — reset dirty
}
...
async function saveModal() {
  ...
  massIsDirty = false; // saved — no longer dirty
  closeModal();
  const mass = (await sb('mass_services', 'GET', null, `?id=eq.${currentMassId}&select=*`))[0];
  renderDetail(mass);
}
