...
async function renderMassList() {
  ...
  try {
    const masses = await sb('mass_services', 'GET', null, '?select=*&order=date.desc');
...
async function renderDetail(mass) {
  ...
  function partSortKey(part) {
    // Direct match in MASS_PARTS — but check if it's a numbered part first
    const numberedMatch = part.match(/^(.+?)\s+(\d+)$/);
    if (numberedMatch) {
      const base   = numberedMatch[1];    // e.g. "Communion"
      const num    = parseInt(numberedMatch[2]); // e.g. 3
      // Find the position of "Base 1" in MASS_PARTS to anchor all variants
      const anchor = MASS_PARTS.findIndex(p => p === base + ' 1' || p === base + ' 2');
      const basePos = anchor !== -1 ? anchor : MASS_PARTS.length;
      return basePos * 1000 + num;
    }
    const idx = MASS_PARTS.indexOf(part);
    return idx !== -1 ? idx * 1000 : 9999;
  }
...
function showAddPartModal() {
  const EXTRA_PARTS = ['Communion', 'Psalm', 'Acclamation', 'Offertory'];
  document.getElementById('addPartChips').innerHTML = EXTRA_PARTS.map(p => `
...
async function confirmAddPart(basePart) {
  ...
  const nums = existing
    ...
    .filter(Boolean)
    .map(m => parseInt(m[1]));
  const bareCount = existing.filter(s => s.part === basePart).length;
  const maxNum = nums.length ? Math.max(...nums) : (bareCount ? 1 : 0);
  const newPart = `${basePart} ${maxNum + 1}`;
...
async function copyMassList() {
  ...
  try {
    ...
    // Try modern clipboard API first, fall back for mobile
    let copied = false;
    ...
    if (!copied) {
      // Mobile fallback — create a temporary textarea
    ...
    btn.textContent = '📋 Copy Song List'; btn.disabled = false;
  }
}
